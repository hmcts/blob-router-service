...
class SummaryReportServiceTest {
    ...
    void should_continue_if_there_is_no_supplier_statement() throws JsonProcessingException {
        ...
        given(supplierStatementRepository.findLatestCreatedByDate(date)).willReturn(Optional.empty());

        // when
        summaryReportService.process(date);

        // then
        verify(supplierStatementRepository).findLatestCreatedByDate(date);
    ...
    void should_stop_process_if_reports_already_ready() throws JsonProcessingException {
        ...
        given(supplierStatementRepository.findLatestCreatedByDate(date)).willReturn(option);
